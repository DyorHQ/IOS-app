...
# Moment struct: (creator, platform, treasury, coin, nft, price, threshold, rateNum, rateDen, creatorBps, platformBps, reserveBps, creatorAllocBps, expiryCreatorBps, publishedAt, deadline)
read -r CREATOR _ _ COIN NFT _ < <($CAST call "$FACTORY" "getMoment(uint256)((address,address,address,address,address,uint256,uint256,uint256,uint256,uint16,uint16,uint16,uint16,uint16,uint64,uint64))" "$ID" --rpc-url $RPC | tr -d '(),' )
NAME=$($CAST call "$COIN" "name()(string)" --rpc-url $RPC | sed 's/^"//; s/"$//')
SYMBOL=$($CAST call "$COIN" "symbol()(string)" --rpc-url $RPC | sed 's/^"//; s/"$//')
# Provenance struct: (mediaURI, mediaHash, place, date)
PROV=$($CAST call "$NFT" "provenance()((string,bytes32,string,uint64))" --rpc-url $RPC)
...
DATE=$(python3 -c "import re,sys; s=sys.argv[1]; print(re.findall(r'(\d+)\)$', s.strip())[0])" "$PROV")
...
NFT_ARGS=$($CAST abi-encode "c(uint256,string,string,address,address,address,(string,bytes32,string,uint64))" "$ID" "$NAME" "$SYMBOL" "$CREATOR" "$COLLECT" "$GRAD" "($MEDIA,$HASH,$PLACE,$DATE)")
